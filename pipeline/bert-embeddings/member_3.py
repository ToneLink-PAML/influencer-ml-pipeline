...
# ---------------------------
influencer_df = pd.read_csv("content/influencer_data.csv")

# Check for column named "description" (you can change this if needed)
if "bio" not in influencer_df.columns:
    raise ValueError("Column 'bio' not found in influencer_data.csv. Please rename your text column.")

influencer_embeddings = []
print("🔹 Generating Influencer Embeddings...")
for text in tqdm(influencer_df["bio"].fillna("")):
...
np.save("embeddings/influencer_embeddings.npy", influencer_embeddings)
print("✅ Influencer embeddings saved as influencer_embeddings.csv & influencer_embeddings.npy")

# ---------------------------
# 3️⃣ Process Brand Data
# ---------------------------
brand_df = pd.read_csv("content/brand_data.csv")

if "campaign_summary" not in brand_df.columns:
...
for text in tqdm(brand_df["campaign_summary"].fillna("")):
...
np.save("embeddings/brand_embeddings.npy", brand_embeddings)
